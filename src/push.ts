...
// rule is what makes a falloff mean anything here).
//
...
import { JointId, JOINT_IDS, MAX_REACH, jointBound } from './skeleton';
 ...
 * travel (`dViewX`, `dViewY`) scaled by {@link pushFalloff}. Coordinates
 * and radius are VIEW units — what `projectTurn` emits and what
 * `resolveDrag` already takes.
 *
 ...
 *
 * The ROOT is never displaced. It is the figure's anchor rather than part
 * of its shape: the view pivots on it, the stage is measured from it, and
 * moving the whole figure is already what dragging the body (or its root
 * knob) does. Leaving it out keeps the pivot still under the brush and
 * keeps one invariant simple — every displaced joint is clamped into the
 * ball the STAGE guarantees to frame (`MAX_REACH`, less the flesh drawn
 * past the joint and less however far the root has already wandered), so
 * no amount of pushing can smear the figure out through its own viewport.
 *
...
export function pushPose(
  ...
  for (const id of JOINT_IDS) {
    if (id === 'root') continue; // the anchor, not the shape — see above
    const j = world[id];
