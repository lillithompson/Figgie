...
// the canvas under the current yaw + fit, nearest one within the touch
...
import { Fit, projectYaw } from './view';
...
export interface Hit {
  target: DragTarget;
  /** Finger − joint at grab time, in VIEW coordinates (the yawed frame
...
export function hitTest(
  ...
  for (const target of DRAG_TARGETS) {
    ...
    const p = projectYaw(j.x, j.y, j.z, yaw, pivotX);
